    ...
    public partial class MainForm : Form
    {
        ...
        private async Task ConvertPresentation(string pdfPath, string exportPath)
        {
            ...
            for (int i = 0; i < pageCount; i++)
            {
                ...
                var tempPath = Path.Combine(Path.GetTempPath(), "TT");
                if (!Directory.Exists(tempPath))
                    Directory.CreateDirectory(tempPath);

                var imgFilePath = Path.Combine(tempPath, $"img.png");

                if (File.Exists(imgFilePath))
                    File.Delete(imgFilePath);

                using (Stream imgFile = File.Open(imgFilePath, FileMode.CreateNew))
                {
                ...
                shape.Image.SetImage(imgFilePath);
